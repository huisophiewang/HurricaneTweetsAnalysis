...
def query_api(api, date, max_id=None):
    ...
    q_str = r'HurricaneMatthew OR "Hurricane Matthew" since:%s until:%s' % (since_dt, until_dt)
...
def write_to_json(tweets):  
    for tweet in tweets:
        ...
        f_out = open(os.path.join(dir, file_name), 'a')
...
def search(dt): 
    ...
    write_to_json(first_query[:1])   
    while True:
        try:
            files = os.listdir(dir)
            latest_file = sorted(files)[0]   
            f_latest_file = open(os.path.join(dir, latest_file))
